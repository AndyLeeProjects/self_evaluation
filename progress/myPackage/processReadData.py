...
if os.name == 'nt':
    path = 'D:\Personal\progress\Data\%s.csv'
else:
...
class read_data():
    ...
    def all_data(self):
        year = 20
        month = 9
        months_key = [] # all data in monthly segments
        while True:
            try:
                if month == 13:
                    month = 1
        ...
        all_dat = pd.DataFrame(pCor.CorSetUp(months_key))
...
all_data, months_key = mon.all_data()

    

# How to view the whole dataframe in pandas
